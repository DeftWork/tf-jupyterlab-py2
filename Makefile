NAME ?= elswork/tf-jupyterlab-py2

build:
	docker build --no-cache -t $(NAME):latest --build-arg BUILD_DATE=`date -u +"%Y-%m-%dT%H:%M:%SZ"` \
	--build-arg VCS_REF=`git rev-parse --short HEAD` \
	--build-arg VERSION=latest-`cat VERSION` . > ../builds/tf-juplab-py2_`date +"%Y%m%d_%H%M%S"`.txt
tag:
...
build-arm:
	...
	--build-arg VCS_REF=`git rev-parse --short HEAD` \
	--build-arg VERSION=arm32v7-`cat VERSION` . > ../builds/tf-juplab-py2-arm_`date +"%Y%m%d_%H%M%S"`.txt
tag-arm:
